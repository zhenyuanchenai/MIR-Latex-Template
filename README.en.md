...
- Provides robust biography helpers, `mirbiographies` and `mirbiography`, for multi-author photo biography pages.
- `mir.bst` supports `doi` and `url` fields for DOI, arXiv, OpenReview, and official online links.
...
For long affiliations, use a compact first-page form such as "Institute,
University, City postcode, Country"; full postal addresses can be kept for the
submission system or author information page. Keep `\\[3pt]` between
affiliations so the title block does not become too tall. A corresponding author
note can be placed in the biography or affiliation text, for example
`Email: author@example.edu (corresponding author).`
...
- Use single-column `figure` / `table` for small material and `figure*` / `table*` for wide material.
- Prefer `[!t]` or `[!tb]` so floats stay near their first discussion and naturally appear at the top of a page.
- Avoid `wrapfigure` / `wraptable` for main manuscript figures and tables; they often narrow the text column and create uneven two-column pages.
- Avoid negative `\vspace` for page squeezing. Adjust figure width, table font size, `\tabcolsep`, `\arraystretch`, or caption length instead.
- For wide tables, use `adjustbox` with `max width=\textwidth` or `max width=\columnwidth` to prevent margin overflow.
...
`mir.bst` prints the `doi` and `url` fields when they are present. For formally
published papers, use full conference or journal names and add pages, year, and
DOI when available. For arXiv, OpenReview, CVF Open Access, or other online-only
records without a formal DOI, use `url`:
...
The template still supports the IEEE-style `IEEEbiography` environment. Author
photos should be placed in `figures/`.
...
For multi-author papers, the recommended interface is `mirbiographies` /
`mirbiography`. It starts a two-column biography page and measures the photo
height automatically, reducing the risk of photo/text overlap:
...
If a photo has excessive horizontal whitespace, crop it locally with LaTeX
`trim` / `clip` options instead of editing the source image:
...
For unusually long biographies, tune `\mirwrapphotoheight`, `\mirwrapbiofont`,
`\mirwrapbiolines`, or `\mirwrapbiosep` locally. Most manuscripts should not
need to modify the template source.
...
The LaTeX template files and documentation in this repository are released
